...
use rate_limiter_aimd::{
    adaptive_concurrency::{
        ...
        retries::{
            RetryAction, RetryLogic, JitterMode, ExponentialBackoff, // Added ExponentialBackoff
        ...
        let handle = tokio::spawn(async move {
            ...
            match cloned_service.call(http_request).await {
                Ok(response) => {
                    ...
                    if status.is_success() {
                        info!("[T{}] SUCCESS: {} Body: {}", task_id, status, response_body_text);
                    } else {
                        warn!("[T{}] API_ERR_FINAL: {} Body: {}", task_id, status, response_body_text);
                    }
                }
