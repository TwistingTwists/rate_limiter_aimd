...
use tower::{ServiceBuilder, Service, ServiceExt, retry::RetryLayer}; // Ensure RetryLayer is here
use tracing::{info, warn, error, Level, debug}; // Added debug
use serde_json::json;
use tracing_subscriber::{fmt, EnvFilter, layer::SubscriberExt, util::SubscriberInitExt};
use tracing_appender;
use bytes::Bytes; // <<< ADD THIS
...
const REQUEST_INTERVAL_MS: u64 = 20; // How quickly to spawn new tasks.
const DEFAULT_MODEL_NAME: &str = "Qwen/Qwen3-235B-A22B-FP8";

const MAX_INDIVIDUAL_REQUEST_RETRIES: usize = 5;
const INITIAL_RETRY_BACKOFF_SECS: u64 = 2; // Start with a slightly higher backoff
...
impl RetryLogic for OpenAIRetryLogic {
    ...
    fn is_retriable_error(&self, error: &Self::Error) -> bool {
        match error {
            ...
            GenericHttpError::InvalidRequest { .. } | GenericHttpError::BuildRequest { .. } | GenericHttpError::ClientError { .. }=> {
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>> {
    ...
    let default_filter = "info,rate_limiter_aimd::adaptive_concurrency::stats=debug";
    ...
    info!("Tracing initialized. Default filter: '{}'. Logging to console and {}", default_filter, LOG_FILE_NAME);
    dotenvy::dotenv().ok();
    info!("Attempted to load .env file");

    let api_key = env::var(OPENAI_API_KEY_ENV_VAR).map_err(|e| format!("Missing OpenAI API key env var: {}: {}", OPENAI_API_KEY_ENV_VAR, e))?;
    ...
    let ac_settings = AdaptiveConcurrencySettings::builder()
        ...
        .max_concurrency_limit(10) // Lowered max for testing rate limits sooner
        .ewma_alpha(0.3)
        .decrease_ratio(0.75) // More aggressive decrease
        .rtt_deviation_scale(1.5) // More sensitive to RTT increases
        .build();
    info!(target: "config", adaptive_concurrency_settings = ?ac_settings);

    let openai_retry_logic = OpenAIRetryLogic::default();

    // Retry Policy for RetryLayer (operates on ReqwestService's error type: GenericHttpError)
    ...
    // Adaptive Concurrency Layer (operates on the error type from the retrying service, which becomes CrateError)
    ...
    // VVV CORRECTED SERVICE LAYERING VVV
    let retrying_reqwest_service = ServiceBuilder::new()
        .layer(RetryLayer::new(retry_policy)) // Retries individual requests to ReqwestService
        .service(reqwest_service);

    // ACL wraps the service that already has retry logic built in.
    // The error type from retrying_reqwest_service will be tower::retry::Error<GenericHttpError, GenericHttpError>,
    // which needs to be convertible to CrateError for AdaptiveConcurrencyLimit.
    ...
    for (i, prompt_content) in prompts.into_iter().enumerate() {
        ...
        let handle = tokio::spawn(async move {
            ...
            let http_request = HttpRequest::builder().method("POST").uri(url_clone.as_str())
                ...
                .body(Some(body_bytes)) // Pass Option<Bytes>
                .map_err(|e| CrateError::from(format!("[T{}] BuildReqErr: {}", task_id, e)))?;

            debug!("[T{}] Waiting for service readiness...", task_id);
            // The error from cloned_service.ready() is CrateError due to ACL's mapping
            if let Err(e) = cloned_service.ready().await {
                 let err_msg = format!("[T{}] NotReady: {:?}", task_id, e);
                 error!("{}", err_msg); return Err(CrateError::from(err_msg)); // Ensure CrateError is returned
            }

            info!("[T{}] Calling service...", task_id);
            // The error from cloned_service.call() is CrateError
            match cloned_service.call(http_request).await {
                Ok(response) => {
                    ...
                    if status.is_success() {
                        ...
                        warn!("[T{}] API_ERR_FINAL: {} Body: {}", task_id, status, response_body_text); // Final API error after retries
                    }
                }
                Err(e) => error!("[T{}] SVC_ERR_FINAL: {:?}", task_id, e), // Final service error after retries
            }
            Ok::<(), CrateError>(())
        });
