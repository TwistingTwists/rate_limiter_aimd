...
use std::{borrow::Cow, time::Duration, fmt, sync::Arc, env}; // Added env for CARGO_PKG_VERSION
use tower::{ServiceBuilder, Service, ServiceExt, retry::{RetryLayer, Retry}}; // Added retry::Retry
...
impl Default for OpenAIClientConfig {
    fn default() -> Self {
        ...
        Self {
            ...
            user_agent: Some(format!("rust-openai-client-aimd/{}", option_env!("CARGO_PKG_VERSION").unwrap_or("0.1.0"))),
        }
    }
}
    ...
    fn is_retriable_error(&self, error: &Self::Error) -> bool {
        match error {
            ...
            GenericHttpError::ServerError { status, body } => {
                ...
                if s.is_server_error() || s == StatusCode::TOO_MANY_REQUESTS || s == StatusCode::CONFLICT {
...
impl OpenAIClient {
    pub fn new(config: OpenAIClientConfig) -> Result<Self, OpenAIClientError> {
        ...
        let reqwest_client_to_use = config.reqwest_client.clone().unwrap_or_else(|| {
            reqwest::Client::builder()
                ...
                .build()
                .map_err(|e| OpenAIClientError::Initialization(format!("Failed to build default reqwest client: {}",e)))
    ...
    pub async fn chat_completion(
        ...
        if let Some(ua) = &self.config.user_agent {
             http_req_builder = http_req_builder.header(
                 ...
                 HeaderValue::from_str(ua)
                    .map_err(|e| OpenAIClientError::RequestBuild(format!("Invalid User-Agent: {}", e)))?
             );
        }
        ...
        self.service.ready().await.map_err(|e| OpenAIClientError::ServiceError)?;
        ...
        let response = self.service.call(http_req).await.map_err(|e| OpenAIClientError::ServiceError)?;
...
mod tests {
    ...
    async fn test_chat_completion_client() {
        ...
        let messages = vec![
            ChatMessage { role: "system".to_string(), content: "You are a helpful assistant that provides concise answers.".to_string()},
            ChatMessage { role: "user".to_string(), content: "What is the capital of France in one word?".to_string()},
        ];
